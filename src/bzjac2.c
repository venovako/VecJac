...
#pragma omp parallel default(none) shared(ff,fg,fr,fj,a11,a22,a21r,a21i,n,n_t,cnt,jn)
    ...
    (void)fflush(stdout);
    be[0u] = rdtsc_beg(rd);
    th = imax(th, zjac2t_((const fnat*)&n, a11, a22, a21r, a21i, t, c, ca, sa, l1, l2, p));
    be[1u] = rdtsc_end(rd);
    (void)fprintf(stdout, "%15.9Lf,", tsc_lap(hz, be[0u], be[1u]));
    (void)fflush(stdout);
    wide r = W_ZERO;
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(n,a11,a22,a21r,a21i,t,c,ca,sa,l1,l2,AE,AN,L1,L2,RE) reduction(max:r)
#endif /* _OPENMP */
