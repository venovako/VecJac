...
register const VS s1 = _mm512_sqrt_ps(s2); VSP(s1);                                                                      \
register const VS co = _mm512_div_ps(onef, s1); VSP(co);                                                                 \
_mm512_store_ps((at + i), _mm512_div_ps(VSXOR(t1, as), s1));                                                             \
register const VS L1 = _mm512_div_ps(_mm512_fmadd_ps(t1, _mm512_fmadd_ps(a2, t1, ab), a1), s2); VSP(L1);                 \
_mm512_store_ps((c + i), co);                                                                                            \
register const VS L2 = _mm512_div_ps(_mm512_fmadd_ps(t1, _mm512_fmsub_ps(a1, t1, ab), a2), s2); VSP(L2);                 \
_mm512_store_ps((l1 + i), _mm512_scalef_ps(L1, es));                                                                     \
...
fint sbjac2_(const fint n[static restrict 1], const float a11[static restrict VSL], const float a22[static restrict VSL], const float a21[static restrict VSL], float c[static restrict VSL], float at[static restrict VSL], float l1[static restrict VSL], float l2[static restrict VSL], unsigned p[static restrict 1])
{
#ifndef NDEBUG
  if (IS_NOT_VFPENV)
    return -10;
...
#ifdef S8JACI_LOOP
#error S8JACI_LOOP already defined
#else /* !S8JACI_LOOP */
#define S8JACI_LOOP                                                                                                      \
register VS a1 = _mm512_load_ps(a11 + i); VSP(a1);                                                                       \
register VS a2 = _mm512_load_ps(a22 + i); VSP(a2);                                                                       \
...
fint sbjac2i(const fint n[static restrict 1], const float a11[static restrict VSL], const float a22[static restrict VSL], const float a21[static restrict VSL], float c[static restrict VSL], float at[static restrict VSL], float l1[static restrict VSL], float l2[static restrict VSL], unsigned p[static restrict 1])
{
#ifndef NDEBUG
  if (IS_NOT_VFPENV)
    return -10;
