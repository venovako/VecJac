...
then
	...
	make COMPILER=gcc NDEBUG=3 SAFE=DET,SV2,NRM OPENMP=0 GMP=/opt/gmp MPFR=/opt/mpfr SLEEF=/opt/sleef sleef=0 -j
	cd -
	...
	make COMPILER=gcc COMPILER_SUFFIX=${GNU} NDEBUG=3 SAFE=DET,SV2,NRM OPENMP=0 GMP=/opt/gmp MPFR=/opt/mpfr SLEEF=/opt/sleef sleef=0 -j
	cd -
	make COMPILER=gnu clean all
fi
