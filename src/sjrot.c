...
float sjrot_(const fint n[static restrict 1], float x[static restrict VSL], float y[static restrict VSL], const float t[static restrict 1], const float c[static restrict 1])
{
  ...
  if (*n < 0) { // permute
    ...
    if (*t == 0.0f) {
      if (*c == 1.0f) {
        for (fnat i = 0u; i < n_; i += VSL) {
          ...
          mx = _mm512_max_ps(mx, _mm512_max_ps(VSABS(x_), VSABS(y_)));
    ...
    for (fnat i = 0u; i < n_; i += VSL) {
      ...
      register const VS x_ = _mm512_load_ps(xi);
      register const VS y_ = _mm512_load_ps(yi);
      register const VS x_r = _mm512_mul_ps(_mm512_fnmadd_ps(x_, t_, y_), c_);
      register const VS y_r = _mm512_mul_ps(_mm512_fmadd_ps(y_, t_, x_), c_);
    ...
    for (fnat i = 0u; i < n_; i += VSL) {
      ...
      register const VS x_ = _mm512_load_ps(xi);
      register const VS y_ = _mm512_load_ps(yi);
      register const VS x_r = _mm512_mul_ps(_mm512_fmadd_ps(y_, t_, x_), c_);
      register const VS y_r = _mm512_mul_ps(_mm512_fnmadd_ps(x_, t_, y_), c_);
      mx = _mm512_max_ps(mx, _mm512_max_ps(VSABS(x_r), VSABS(y_r)));
      _mm512_store_ps(xi, x_r);
      _mm512_store_ps(yi, y_r);
    }
