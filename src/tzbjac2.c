...
int main(int argc, char *argv[])
{
  ...
  double
    ...
    *const cat = (double*)aligned_alloc(VA, nt),
    *const sat = (double*)aligned_alloc(VA, nt),
...
#pragma omp parallel default(none) shared(ff,fg,fr,fj,a11,a22,a21r,a21i,n,n_t,cnt,jn)
    ...
    wide r = W_ZERO;
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(n,a11,a22,a21r,a21i,c,cat,sat,l1,l2) reduction(max:r)
#endif /* _OPENMP */
    for (size_t i = 0u; i < n; ++i) {
      ...
      const wide RE = wrec(a11[i], a22[i], a21r[i], a21i[i], CS, SNR, SNI, l1[i], l2[i], &AE, &AN);
      r = fmaxw(r, RE);
    }
