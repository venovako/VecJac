...
MODULE VECJAC
  ...
  INTERFACE
     INTEGER FUNCTION DJAC2(N, A11, A22, A21, T, C, L1, L2, P) BIND(C,NAME='djac2_')
  ...
  ! ASSUMES MOD(N,VDL) .EQ. 0
  INTERFACE
     INTEGER FUNCTION ZJAC2(N, A11, A22, A21R, A21I, T, C, CA, SA, L1, L2, P) BIND(C,NAME='zjac2_')
       USE, INTRINSIC :: ISO_C_BINDING, ONLY: C_INT
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
  ...
  ! ASSUMES (MOD(N,2) .EQ. 0) .AND. MOD(N,VDL) .EQ. 0
  INTERFACE
  ...
  INTERFACE
     INTEGER FUNCTION ZVJSVD(M,N, GR,LDGR,GI,LDGI, VR,LDVR,VI,LDVI, ES,FS, JS,STP,SWP, WORK,IWORK) BIND(C,NAME='zvjsvd_')
       ...
       DOUBLE PRECISION, INTENT(OUT) :: ES, FS, WORK(11*(MAX(N,0)/2))
       INTEGER(KIND=C_INT), INTENT(OUT) :: IWORK(MAX(N,VDL)/VDL)
     END FUNCTION ZVJSVD
  END INTERFACE
