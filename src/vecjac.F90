...
MODULE VECJAC
  ...
  INTERFACE
     INTEGER FUNCTION DJAC2(N, A11, A22, A21, S, T, C, L1, L2, P) BIND(C,NAME='djac2_')
       USE, INTRINSIC :: ISO_C_BINDING, ONLY: C_INT
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       DOUBLE PRECISION, INTENT(IN) :: A11(MAX(N,VDL)), A22(MAX(N,VDL)), A21(MAX(N,VDL))
       DOUBLE PRECISION, INTENT(OUT) :: S(MAX(N,VDL)), T(MAX(N,VDL)), C(MAX(N,VDL))
  ...
  ! ASSUMES MOD(N,VDL) .EQ. 0
  INTERFACE
     INTEGER FUNCTION ZJAC2(N, A11, A22, A21R, A21I, S, T, C, CA, SA, L1, L2, P) BIND(C,NAME='zjac2_')
       USE, INTRINSIC :: ISO_C_BINDING, ONLY: C_INT
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
  ...
  INTERFACE
     INTEGER FUNCTION DVJSVD(M, N, G, LDG, V, LDV, ES, FS, JS, STP, SWP, WORK, IWORK) BIND(C,NAME='dvjsvd_')
       ...
       DOUBLE PRECISION, INTENT(OUT) :: ES, FS, WORK((VDL/2)*MAX(N,0))
       INTEGER(KIND=C_INT), INTENT(OUT) :: IWORK(MAX(N,VDL)/VDL)
     END FUNCTION DVJSVD
  END INTERFACE
  ...
  INTERFACE
     INTEGER FUNCTION ZVJSVD(M,N, GR,LDGR,GI,LDGI, VR,LDVR,VI,LDVI, ES,FS, JS,STP,SWP, WORK,IWORK) BIND(C,NAME='zvjsvd_')
       ...
       DOUBLE PRECISION, INTENT(OUT) :: ES, FS, WORK(11*((MAX(N,0)+MOD(MAX(N,0),2))/2))
       INTEGER(KIND=C_INT), INTENT(OUT) :: IWORK(MAX(N,VDL)/VDL)
     END FUNCTION ZVJSVD
  END INTERFACE
