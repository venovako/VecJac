...
MODULE VECJAC
  ...
  INTERFACE
     INTEGER FUNCTION SJROTF(N, X, Y, C, AT) BIND(C,NAME='sjrotf_')
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       REAL, INTENT(IN) :: C, AT
       REAL, INTENT(INOUT) :: X(MAX(N,VSL)), Y(MAX(N,VSL))
  ...
  INTERFACE
     INTEGER FUNCTION CJROTF(N, XR, XI, YR, YI, C, CAT, SAT) BIND(C,NAME='cjrotf_')
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       REAL, INTENT(IN) :: C, CAT, SAT
       REAL, INTENT(INOUT) :: XR(MAX(N,VSL)), XI(MAX(N,VSL)), YR(MAX(N,VSL)), YI(MAX(N,VSL))
  ...
  INTERFACE
     INTEGER FUNCTION SVJSVD(M, N, G, LDG, V, LDV, ES, FS, JS, STP, SWP, WORK, IWORK) BIND(C,NAME='svjsvd_')
       ...
       INTEGER(KIND=C_INT), INTENT(OUT) :: IWORK(MAX(N,0)/VSL)
     END FUNCTION SVJSVD
  END INTERFACE
