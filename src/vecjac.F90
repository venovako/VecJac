...
MODULE VECJAC
  ...
  ! ASSUMES (MOD(N,VDL) .EQ. 0)
  INTERFACE
     INTEGER FUNCTION DJAC2S(N, A11, A22, A21, T, C, L1, L2, P, S) BIND(C,NAME='djac2s_')
       USE, INTRINSIC :: ISO_C_BINDING, ONLY: C_INT
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       DOUBLE PRECISION, INTENT(IN) :: A11(MAX(N,VDL)), A22(MAX(N,VDL)), A21(MAX(N,VDL))
       ...
       DOUBLE PRECISION, INTENT(OUT) :: L1(MAX(N,VDL)), L2(MAX(N,VDL))
       INTEGER(KIND=C_INT), INTENT(OUT) :: P(MAX(N,VDL)/VDL)
       DOUBLE PRECISION, INTENT(OUT) :: S(MAX(N,VDL))
     END FUNCTION DJAC2S
  END INTERFACE
  ...
  ! ASSUMES MOD(N,VDL) .EQ. 0
  INTERFACE
     INTEGER FUNCTION ZJAC2S(N, A11, A22, A21R, A21I, T, C, CA, SA, L1, L2, P, S) BIND(C,NAME='zjac2s_')
       USE, INTRINSIC :: ISO_C_BINDING, ONLY: C_INT
       IMPLICIT NONE
       INTEGER, INTENT(IN) :: N
       DOUBLE PRECISION, INTENT(IN) :: A11(MAX(N,VDL)), A22(MAX(N,VDL)), A21R(MAX(N,VDL)), A21I(MAX(N,VDL))
