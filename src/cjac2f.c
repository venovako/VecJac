...
register const VS aa = _mm512_hypot_ps(ar, ai); VSP(aa);                                                                 \
_mm512_store_ps((ca + i), VSOR(_mm512_min_ps(_mm512_div_ps(VSABS(ar), aa), onef), VSSGN(ar)));                           \
_mm512_store_ps((sa + i), _mm512_div_ps(ai, _mm512_max_ps(aa, ftm)));                                                    \
...
_mm512_store_ps((t + i), t1);                                                                                            \
register VS C = _mm512_invsqrt_ps(_mm512_fmadd_ps(t1, t1, onef)); VSP(C);                                                \
_mm512_store_ps((c + i), C);                                                                                             \
C = _mm512_mul_ps(C, C);                                                                                                 \
register VS L1 = _mm512_fmadd_ps(t1, _mm512_fmadd_ps(a2, t1, ab), a1);                                                   \
register VS L2 = _mm512_fmadd_ps(t1, _mm512_fmsub_ps(a1, t1, ab), a2);                                                   \
...
fint cjac2f_(const fnat n[static restrict 1], const float a11[static restrict VSL], const float a22[static restrict VSL], const float a21r[static restrict VSL], const float a21i[static restrict VSL], float t[static restrict VSL], float c[static restrict VSL], float ca[static restrict VSL], float sa[static restrict VSL], float l1[static restrict VSL], float l2[static restrict VSL], unsigned p[static restrict 1])
{
#ifndef NDEBUG
  if (IS_NOT_VFPENV)
    return -13;
