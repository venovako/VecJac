...
int main(int argc, char *argv[])
{
  ...
  if (dread2_((const fnat*)&m, (const fnat*)&n, G, (const fnat*)&ldG, &gd))
    return EXIT_FAILURE;
  ...
  if (resizef_(&vd, &l))
    return EXIT_FAILURE;
  if (dwrite2_((const fnat*)&n, (const fnat*)&n, V, (const fnat*)&ldV, &vd))
    return EXIT_FAILURE;
  if (close(vd))
    return EXIT_FAILURE;
  free(V);
  ...
  if (resizef_(&ud, &l))
    return EXIT_FAILURE;
  if (dwrite2_((const fnat*)&m, (const fnat*)&n, G, (const fnat*)&ldG, &ud))
    return EXIT_FAILURE;
  if (close(ud))
    return EXIT_FAILURE;
  free(G);
