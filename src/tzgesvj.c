...
int main(int argc, char *argv[])
{
  ...
  if (zread2_((const fnat*)&m, (const fnat*)&n, G, (const fnat*)&ldG, &gd))
    return EXIT_FAILURE;
  ...
  *(size_t*)cwork1 = (n * (n * sizeof(double complex)));
  ...
  if (resizef_(&vd, (const size_t*)cwork1))
    return EXIT_FAILURE;
  if (zwrite2_((const fnat*)&n, (const fnat*)&n, V, (const fnat*)&ldV, &vd))
    return EXIT_FAILURE;
  if (close(vd))
    return EXIT_FAILURE;
  free(V);
  ...
  *(size_t*)cwork1 = (m * (n * sizeof(double complex)));
  ...
  if (resizef_(&ud, (const size_t*)cwork1))
    return EXIT_FAILURE;
  if (zwrite2_((const fnat*)&m, (const fnat*)&n, G, (const fnat*)&ldG, &ud))
    return EXIT_FAILURE;
  if (close(ud))
    return EXIT_FAILURE;
  free(G);
