...
__global__ void ccjac2(const float *const a11, const float *const a22, const float *const a21r, const float *const a21i, float *const cs, float *const snr, float *const sni, float *const l1, float *const l2)
{
  ...
  int _e1, _e2, _er, _ei;
  (void)frexpf(_a11, &_e1);
  _e1 = FLT_BIG_EXP - _e1;
  (void)frexpf(_a22, &_e2);
  _e2 = FLT_BIG_EXP - _e2;
  (void)frexpf(_a21r, &_er);
  _er = FLT_BIG_EXP - _er;
  (void)frexpf(_a21i, &_ei);
  _ei = FLT_BIG_EXP - _ei;
  ...
  const float _ab = __fmul_rn(_aa, 2.0f);
  const float _ad = __fsub_rn(_a1, _a2);
  const float _t2 = copysignf(fminf(fmaxf(__fdiv_rn(_ab, fabsf(_ad)), 0.0f), FLT_SQRT_HUGE), _ad);
  ...
  const float _cs = __fdiv_rn(1.0f, _s1);
  const float _snr = __fdiv_rn(__fmul_rn(_ar_, _t1), _s1);
  const float _sni = __fdiv_rn(__fmul_rn(_ai_, _t1), _s1);
  const float _l1 = scalbnf(__fdiv_rn(__fmaf_rn(_t1, __fmaf_rn(_a2, _t1, _ab), _a1), _s2), _es);
  const float _l2 = scalbnf(__fdiv_rn(__fmaf_rn(_t1, __fmaf_rn(_a1, _t1, -_ab), _a2), _s2), _es);
  cs[_off] = _cs;
  snr[_off] = _snr;
  sni[_off] = _sni;
  l1[_off] = _l1;
...
__global__ void zcjac2(const double *const a11, const double *const a22, const double *const a21r, const double *const a21i, double *const cs, double *const snr, double *const sni, double *const l1, double *const l2)
{
  const size_t _off = (size_t)(blockIdx.x) * blockDim.x + threadIdx.x;
  ...
  (void)frexp(_a22, &_e2);
  _e2 = DBL_BIG_EXP - _e2;
  (void)frexp(_a21r, &_er);
  _er = DBL_BIG_EXP - _er;
  (void)frexp(_a21i, &_ei);
  _ei = DBL_BIG_EXP - _ei;
  int _es = min(min(_e1, _e2), min(_er, _ei));
  ...
  const double _ab = __dmul_rn(_aa, 2.0);
  const double _ad = __dsub_rn(_a1, _a2);
  const double _t2 = copysign(fmin(fmax(__ddiv_rn(_ab, fabs(_ad)), 0.0), DBL_SQRT_HUGE), _ad);
  ...
  const double _cs = __ddiv_rn(1.0, _s1);
  const double _snr = __ddiv_rn(__dmul_rn(_ar_, _t1), _s1);
  const double _sni = __ddiv_rn(__dmul_rn(_ai_, _t1), _s1);
  const double _l1 = scalbn(__ddiv_rn(__fma_rn(_t1, __fma_rn(_a2, _t1, _ab), _a1), _s2), _es);
  const double _l2 = scalbn(__ddiv_rn(__fma_rn(_t1, __fma_rn(_a1, _t1, -_ab), _a2), _s2), _es);
  cs[_off] = _cs;
  snr[_off] = _snr;
  sni[_off] = _sni;
  l1[_off] = _l1;
...
__global__ void scjac2(const float *const a11, const float *const a22, const float *const a21, float *const cs, float *const sn, float *const l1, float *const l2)
{
  const size_t _off = (size_t)(blockIdx.x) * blockDim.x + threadIdx.x;
  const float _a11 = a11[_off];
  const float _a22 = a22[_off];
...
__global__ void dcjac2(const double *const a11, const double *const a22, const double *const a21, double *const cs, double *const sn, double *const l1, double *const l2)
{
  const size_t _off = (size_t)(blockIdx.x) * blockDim.x + threadIdx.x;
  const double _a11 = a11[_off];
  const double _a22 = a22[_off];
